...
export const asyncData = {
  ...
  fetchTokenListAndDispatchToStore: async (
    state: State,
    dispatch: Dispatch,
  ) => {
  ...
  fetchAccountInfoAndDispatchToStore: async (
    ...
    if (
      shouldAttemptUnlock &&
      providerState.account.state !== AccountState.Loading
    ) {
    ...
    try {
      ...
      if (e.message.includes('Fortmatic: User denied account access.')) {
        ...
        const initialProviderState =
          providerStateFactory.getInitialProviderStateWithCurrentProviderState(
            providerState,
            chainId,
          );
    ...
    if (!_.isEmpty(availableAddresses)) {
      ...
      asyncData.fetchAccountBalanceAndDispatchToStore(
        activeAddress,
  ...
  fetchAccountBalanceAndDispatchToStore: async (
    ...
    try {
      ...
      if (tokenArray.length) {
        ...
        const tokenBalances =
          await MulticallUtils.getTokensBalancesAndAllowances(
            web3Wrapper.getProvider() as any,
            tokenArray,
            chainId,
            address,
            ethBalanceInWei,
  ...
  fetchCurrentApiSwapQuoteAndDispatchToStore: async (
    ...
    const takerAddress =
      providerState.account.state === AccountState.Ready
        ? providerState.account.address
        : '';
    const selectedTokenUnitAmount = isIn
      ? selectedTokenAmountIn
      : selectedTokenAmountOut;
